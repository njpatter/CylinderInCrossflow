classdef CrossflowExperiment < handle 
    ...
    methods
        ...
        function [calibrationData, cylinderData, wakeProfileData] = RunExperiment(obj, angleIncrement, vertIncrement)
            ...
            fprintf("During this initial portion of the experiment, you should collect \n");
            fprintf("anemometer data at multiple wind tunnel velocities so that you can \n");
            fprintf("determine a voltage-velocity relationship\n");
            ...
            while (~strcmp(userIn, "s")) 
                ...
                for i = 1:10
                    ...
                    fprintf("For Speed setting %i : Pressures (Pa): %6.4f, %6.4f, %6.4f, %6.4f and anemometer (Volts): %6.4f\n", sampleCount, pressures, anemometerReading)
                    pause(0.5)
                end
                sampleCount = sampleCount + 1;
                fprintf("\n\n");
                userIn = input("Hit enter to collect more data at a different speed (change speed first), type 's' and hit enter to stop ", 's');
            end
            ...
            for j = 0:numSteps 
                ...
                pause(2);
                [pressures, anemometerReading] = GetAllSensorData(obj, 100);
            ...
            fprintf("calibrationData columns: Speed #, Static-Atm Pressure Diff, Stag-Static Pressure Diff, Cyl-Static Pressure Diff, Extra values\n");
            fprintf("cylinderData columns: Angle, Static-Atm Pressure Diff, Stag-Static Pressure Diff, Cyl-Static Pressure Diff, Extra values\n");
            fprintf("wakeProfileData columns: Vertical Position, Anemometer reading \n")
        end
        ...
        function [pressures, anemometerReading] = GetAllSensorData(obj, averageOver) 
            ...
            SendCommand(obj, commandList);
            responseData = ReadResponse(obj);
            pressures = responseData(1:4);
            anemometerReading = responseData(5) / 8192 * 3.3;
        end
        ...
        function WaitForMoveResponse(obj) 
            ...
            while(toc(serialTimer) < 25) % Enter the while loop for 5 seconds, you will want to change this to match the number of expected samples
                ...
                if contains(data, newline) || contains(data, char(13))
                    if (contains(lower(data), "done moving"))
                        %data;
                        return;
                    end
                end 
