...
float sensorInitVoltages[numPressureSensors] = {2.31, 2.31, 2.31, 2.31};
float minPressureVoltages[numPressureSensors] = {1.47, 1.46, 1.46, 1.46};
float maxPressureVoltages[numPressureSensors] = {3.14, 3.14, 3.15, 3.14};
float pascalsAtMinMaxReading = 497.68; // Currently equates to 2 in H2O to match best gauge in the lab.
...
int sampleFreq = 1000; // Number of sensor readings taken per second 
...
void UpdateAndAveragePressures(int avgCount) {
  ...
  pressureValues[3] = 0.0;
  float tempVoltages[4] = {0.0, 0.0, 0.0, 0.0};
  anemometerValue = 0.0;
  for (int i = 0; i < avgCount; i++) {
    ...
    for(int j = 0; j < numPressureSensors; j++) { 
      ...
      tempSensorReading = ads.readADC_SingleEnded(j); 
      tempVoltages[j] += ads.computeVolts(tempSensorReading) / (float)avgCount;
    }
    delay(1000 / sampleFreq + 1);
  }
  /*for(int j = 0; j < numPressureSensors; j++) { 
    Serial.print(tempVoltages[j]);
    Serial.print(",    ");
  }*/
}
...
float GetPressureReading(int sensorIndex) {
  ...
  tempPressure = (ads.computeVolts(tempSensorReading) - sensorInitVoltages[sensorIndex]);
  if (tempPressure < 0) {
    tempPressure = tempPressure / (sensorInitVoltages[sensorIndex] - minPressureVoltages[sensorIndex]) * pascalsAtMinMaxReading;
  } else {
    tempPressure = tempPressure / (maxPressureVoltages[sensorIndex] - sensorInitVoltages[sensorIndex]) * pascalsAtMinMaxReading;
  }
...
void serialEvent() {
  ...
  while (Serial.available()) {
    ...
    switch ((byte)inByte) {
      ...
      {
        for(int i = 0; i < numPressureSensors; i++) { 
          Serial.print(" Sensor initialization no longer supported: " );
          Serial.print(i);
      ...
      case kSensorSetRate:
      {
        ...
        Serial.print("Sample Rate No Longer Supported: // ");
        Serial.print(0); 
        Serial.println("");
        break;
      }
